...
export function edgeDetection(context) {
  ...
  const imageDataX = applyConvolution(
    context.getImageData(0, 0, context.canvas.width, context.canvas.height),
    sobelX,
  );
  const imageDataY = applyConvolution(
    context.getImageData(0, 0, context.canvas.width, context.canvas.height),
    sobelY,
  );
...
export function asciiFilter(context, { newWidth, chars, font, fill }) {
  return new Promise((resolve) => {
    ...
    const imageData = resizeContext.getImageData(
      0,
      0,
      newWidth,
      newHeight,
...
export function pixelSortFilter(
  context,
  { direction, sortMethod, lowerThreshold, upperThreshold, intervalType },
) {
  const { width, height } = context.canvas;
  const imageData = context.getImageData(0, 0, width, height);
  const pixels = imageData.data;
  ...
  const processSpan = (index, isRow) => {
    ...
    if (start !== -1)
      sortSpan([], start, (isRow ? width : height) - 1, index, !isRow);
  };
...
function createContrastMask(pixels, lowerThreshold, upperThreshold) {
  ...
  for (let i = 0; i < pixels.length; i += 4) {
    ...
    mask[i / 4] =
      luminance >= lowerThreshold && luminance <= upperThreshold ? 1 : 0;
  }
  return mask;
}
...
function getValue(pixels, x, y, width, sortMethod) {
  ...
  switch (sortMethod) {
    ...
    case "laplace":
      return getLaplace(pixels, width, x, y);
    case "luminance":
      return RGBToHSL(r, g, b)[2];
    case "rgb":
      return 0.298 * r + 0.587 * g + 0.114 * b;
    default:
      return 0;
  }
}
